...
//            excelOperations.setActiveSheet(ClassClosedXML.Sheets.BatimMortgage);
            ...
            foreach (Classbatim batim in allBatim)
            {
                ...
                for (int i = 0; i < batim.tatHelkot.Count; i++)
                {
                    ...
                    if (firstRow) excelOperations.PutValueInSheetRowColumn(ClassClosedXML.Sheets.BatimMortgage, startmainSection, 1, batim.header.gush);
                    if (firstRow) excelOperations.PutValueInSheetRowColumn(ClassClosedXML.Sheets.BatimMortgage, startmainSection, 2, batim.header.helka);
                    excelOperations.PutValueInSheetRowColumn(ClassClosedXML.Sheets.BatimMortgage, startmainSection, 3, batim.tatHelkot[i].number.ToString());
                    if (firstRow) excelOperations.PutValueInSheetRowColumn(ClassClosedXML.Sheets.BatimMortgage, startmainSection, 12, batim.PDFFileName);
                    firstRow = false;
                    for (int j = 0; j < batim.tatHelkot[i].mortgageTatHelkas.Count; j++)
                    {
                        ...
                        if (mort.mortRemarks.Count > 0)
                        {
                            ...
                            {
                                excelOperations.PutValueInSheetRowColumn(ClassClosedXML.Sheets.BatimMortgage, startRemarkSection, 11, mort.mortRemarks[k]);
                                startRemarkSection++;
                            }
                            startRemarkSection--;
                        }
